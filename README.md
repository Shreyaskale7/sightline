...
> retrieval **Recall@5 0.752** on a benchmark spanning all 15 companies (**+108%** over the
...
- **Retrieval** — dense (BGE) + chunking + metadata filtering + cross-encoder reranking, fused
  ...
  measured. Ablation across every configuration → **champion Recall@5 0.752** across 15 companies.
...
scoring; and on a benchmark **extended to all 15 companies** the champion is **0.752** — a domain
prior (prefer the income-statement page for figure questions) lifted 0.582 → 0.731, then RRF-fusing
the cross-encoder with the dense rank took it to 0.752 after a diagnostic showed the reranker was
demoting pages the dense retriever had correctly found. Full numbers, the ablation table, and
reproduce commands: [`docs/RESULTS.md`](docs/RESULTS.md).
